...
export default function ItemEditDrawer() {
  ...
  const isDrawerOpen = useItemDialogStore((state) => state.isOpen && state.type === 'edit');
  const item = useItemDialogStore((state) => state.item);
  ...
  const mutation = useMutation({
    mutationFn: (value: { name: string }) => {
      return axiosInstance.put(`/api/items/${item?.id}`, value);
    },
    onSuccess: async () => {
      ...
      <DrawerContent className="!bottom-0 mt-auto !h-fit overflow-hidden overflow-y-auto rounded-t-2xl px-4 py-6 !pt-0 !pb-12">
        <form onSubmit={handleSave}>
          <DrawerHeader className="sr-only">
            ...
            <DrawerDescription>Edit item here.</DrawerDescription>
          </DrawerHeader>
